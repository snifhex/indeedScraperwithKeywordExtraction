"""
Retrieve job listings according to the job title and location from indeed.co.in

Usage:
    python3 indeedIndia.py <job> <location>

    note: If command line argument not given, script will use defautl for job = "Data+Scientist"
        and loc = "Delhi".
...
def getTotalResults(job, loc):
    ...
    resp = requests.get(
        f"https://www.indeed.co.in/jobs?q={job}&l={loc}")
...
def scraper(total_results, job, loc):
    """
    Gets all the job data from the site and saves in the "indeedIndiaScraping.csv".
    ...
    for offset in range(0, total_results, 15):
        url = f"https://www.indeed.co.in/jobs?q={job}&l={loc}&start={offset}"
...
def writeToCsv(contents):
    ...
    """
    with open("indeedIndiaScraping.csv", 'a') as csv_writer:
...
if __name__ == "__main__":
    ...
    if not os.path.exists("indeedIndiaScraping.csv"):
        with open("indeedIndiaScraping.csv", 'w') as csv_writer:
            fields = ['title', 'company', 'job_location',
                      'experience', 'skills', 'link', 'keywords (Yake)', 'keywords (Rake)', 'keywords ( StackOverflow Tags)']
            writer = csv.DictWriter(csv_writer, fieldnames=fields)
            writer.writeheader()
